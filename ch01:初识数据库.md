...
```
执行 `SHOW DATABASES` 可以查询到所有的数据库，可以看到自己创建的数据库 `shop`。

在创建表之前，首先需要选定数据库，就是说你要在哪个数据库里创建表，语法是 `USE DATABASE_NAME`。

咱们这里就选择刚刚创建的 `shop` 数据库：
...
执行 `SHOW TABLES` 可以查询到所有的表，可以看到自己创建的数据库 `product`。
